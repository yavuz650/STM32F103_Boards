...
int main(void)
{
  ...
  MX_SPI1_Init();
	...
	uint8_t sensorbuf[60];
	
  while (1)
  {
		
		sprintf((char*)sensorbuf,"Altitude: %0.3fm\nPressure:%0.0fPa\nTemperature:%0.4f\n",Return_Altitude(&altimeter),Return_Pres(&altimeter),Return_Temp(&altimeter));
		res=HAL_UART_Transmit(&huart1,sensorbuf,60,20);
		if(Return_Altitude(&altimeter)<=99)
		{
